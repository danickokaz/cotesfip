...
{
    // Fermeture du précédent lecteur si nécessaire
    ...
    // Vérifications avant d'ouvrir le fichier
    if (empty($this->WorksheetPath)) {
        throw new Exception("Erreur : WorksheetPath est vide.");
    }
    if (!file_exists($this->WorksheetPath)) {
        throw new Exception("Erreur : Le fichier spécifié est introuvable : " . $this->WorksheetPath);
    }

    try {
        $this->Worksheet->open($this->WorksheetPath);
    } catch (Exception $e) {
        throw new Exception("Erreur lors de l'ouverture du fichier XML : " . $e->getMessage());
    }
