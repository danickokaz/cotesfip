...
if(isset($_POST['nomutilisateur'],$_POST['motdepasse']) and !empty($_POST['nomutilisateur']) and !empty($_POST['motdepasse'])){
    ...
    $req = database()->prepare("SELECT token_utilisateur FROM dgrad_utilisateur WHERE pseudo = ? AND motdepasse=? AND id_etat_utilisateur=?");
    ...
    if($req->rowCount()==1){
        ...
        $req = database()->prepare("UPDATE dgrad_utilisateur SET token_utilisateur=? WHERE token_utilisateur=?");
        $req->execute([$tokenGenere,$tokenRecuperer]);
        session_regenerate_id(true);
        $_SESSION['jlk'] = $tokenGenere;
        echo "Success";
    }else{
