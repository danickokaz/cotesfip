...
if(isset($_POST['nomutilisateur'],$_POST['motdepasse']) and !empty($_POST['nomutilisateur']) and !empty($_POST['motdepasse'])){
    ...
    $req = database()->prepare("SELECT token_utilisateur FROM dgda_utilisateur WHERE pseudo = ? AND motdepasse=? AND id_etat_utilisateur=?");
    ...
    if($req->rowCount()==1){
        ...
        $req = database()->prepare("UPDATE dgda_utilisateur SET token_utilisateur=? WHERE token_utilisateur=?");
        $req->execute([$tokenGenere,$tokenRecuperer]);
        session_regenerate_id(true);
        $_SESSION['access'] = $tokenGenere;
        echo "Success";
    }else{
